...
cstruct.unpackSync = function(name, buffer, options) {
  ...
  function next(pointer, buffer, scheme) {
    var res = {};
    ...
    for (var el in scheme) {
      ...
      if (typeof s === 'string') {
        ...
        if (s.indexOf('string') > -1) {
          var xstring = s.split(':');
          res[el] = unpacker[xstring[0]](pointer, buffer, parseInt(xstring[1]));
        }
        // number
        else {
          res[el] = unpacker[s + byteord](pointer, buffer);
        }
      } else if (Array.isArray(s)) {
        res[el] = list(pointer, buffer, s[0]);
      } else {
        res[el] = next(pointer, buffer, s);
      }
    }
  ...
  function list(pointer, buffer, scheme) {
    ...
    while (count--) {
      res.push(next(pointer, buffer, scheme));
    }
  ...
  function next(obj, scheme) {
    ...
    for (var el in scheme) {
      ...
      if (typeof s === 'string') {
        ...
        if (s.indexOf('string') > -1) {
          var xstring = s.split(':');
          binary.push(packer[xstring[0]](obj[el], parseInt(xstring[1])));
        }
        // number
        else {
